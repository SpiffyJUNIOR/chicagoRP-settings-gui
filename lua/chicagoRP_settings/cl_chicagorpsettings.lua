...
local function CreateCategoryButton(name, x, y, parent) -- GOING DEATH CON 3 ON FEDS TOMORROW
    local categoryButton = vgui.Create("DButton", parent)
    categoryButton:SetPos(x, y)
    categoryButton:SetSize(394, 56)
    categoryButton:SetFont("MichromaRegular")
    categoryButton:SetText("")
    categoryButton:SetTextColor(primarytext)
    ...
    function categoryButton:Paint(w, h)
        if self:IsHovered() and !settingsScrollPanel:IsVisible() then
            surface.SetDrawColor(34, 34, 34, 100)
            surface.DrawRect(0, 0, w, h)
        elseif !self:IsHovered() and settingsScrollPanel:IsVisible() then
...
local function CreateSettingsButton(printname, convar, helptext, parent)
    ...
    function settingsButton:Paint(w, h)
        ...
        if (GetConVar(convar):GetInt() == 1) and (v.max == 1) then -- add float check pls
            statusString = "Enabled"
        elseif (GetConVar(convar):GetInt() > 1) and (v.max > 1) then
            statusString = GetConVar(convar):GetInt()
        end
    ...
    function settingsButton:DoClick()
        ...
        local _,ScreenY = settingsButton:LocalToScreen()
